...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(srcdir)/package.json.in $(top_srcdir)/lib/version.js.in \
	$(top_srcdir)/test/runner.in $(top_srcdir)/test/perf/runner.in \
	AUTHORS COPYING ChangeLog NEWS README tools/install-sh \
	tools/mdate-sh tools/missing tools/texinfo.tex \
...
CONFIG_CLEAN_FILES = package.json lib/version.js test/runner \
	test/perf/runner
...
lib/version.js: $(top_builddir)/config.status $(top_srcdir)/lib/version.js.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
test/runner: $(top_builddir)/config.status $(top_srcdir)/test/runner.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
test/perf/runner: $(top_builddir)/config.status $(top_srcdir)/test/perf/runner.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@	cat $(path_tools)/license-min.tpl > $@
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@	$(JAVA) -jar $(compiler) \
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@		--externs $(path_tools)/externs-global.js \
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@		--externs $(path_build)/externs-internal.js \
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@		--language_in=ECMASCRIPT5_STRICT \
@HAS_CCJAR_TRUE@@HAS_JAVA_TRUE@		--js $< >> $@ || rm $@
@HAS_CCJAR_FALSE@@HAS_JAVA_TRUE@	@echo "cannot create $@:"
@HAS_CCJAR_FALSE@@HAS_JAVA_TRUE@	@echo "  please download closure-compiler.jar and re-run ./configure"
@HAS_CCJAR_FALSE@@HAS_JAVA_TRUE@	@exit 1
@HAS_JAVA_FALSE@	@echo "Please install a Java virtual machine (e.g. openjdk)"
@HAS_JAVA_FALSE@	@exit 1
...
perf: perf.log
perf.%: FORCE
@HAS_NODE_TRUE@	@$(path_test)/perf/runner @PERF_TESTS@ > "$@"
@HAS_NODE_FALSE@	@echo "Node.js must be installed in order to run performance tests"
@HAS_NODE_FALSE@	@exit 1
...
# JS test cases
test-suite:
@HAS_NODE_TRUE@	@echo "GNU ease.js Test Suite"
@HAS_NODE_TRUE@	@echo
@HAS_NODE_TRUE@	@(cd $(path_test) && ./runner $(test_cases))
@HAS_NODE_FALSE@	@echo "Node.js must be installed in order to run the test suite"
@HAS_NODE_FALSE@	@exit 1
